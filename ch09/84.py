def main():
    ...
    word2vec_model = KeyedVectors.load_word2vec_format(path.join(path.dirname(path.abspath(__file__)), fp_model), binary=True)
    ...
    embedding = torch.zeros(len(word_ids) + 1, embedding_dim)
    for word, i in word_ids.items():
        try:
            embedding[i, :] = torch.tensor(word2vec_model[word])
        except:
            pass
