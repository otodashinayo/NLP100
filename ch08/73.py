def main():
    ...
    if not path.isdir(path.join(path.dirname(path.abspath(__file__)), "73")):
        mkdir(path.join(path.dirname(path.abspath(__file__)), "73"))
    torch.save(model.state_dict(), path.join(
        path.dirname(path.abspath(__file__)), "73/model.pth"))
