def main():
    ...
    for t in s:
        for src, chunk in t.items():
            ...
            t[src] = Chunk(t[src])
    for src, chunk in s[4].items():
        if chunk.dst != "-1" and chunk.contain_verb:
            particles = []
            texts = []
            for src in chunk.srcs:
                if s[4][src].contain_particle_2:
                    particles.append(s[4][src].particles_2[0])
                    texts.append(s[4][src].text)
            if len(particles) > 0:
                print(chunk.verbs[0] + "\t" + " ".join(particles) + "\t" + " ".join(texts))
