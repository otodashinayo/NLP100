    ...
    for (src, chunk_1), (src, chunk_2) in combinations(s[13].items(), 2):
        if chunk_1.contain_noun and chunk_2.contain_noun:
            ...
            if len(texts_1) > 0 and len(texts_2) == 0 and len(texts_3) > 0:
                print(" -> ".join([" -> ".join([t.text_X if i == 0 else t.text for i, t in enumerate(texts_1)]), texts_3[0].text_Y]))
            elif len(texts_1) > 0 and len(texts_2) > 0 and len(texts_3) > 0:
                print(" | ".join([" -> ".join([t.text_X if i == 0 else t.text for i, t in enumerate(texts_1)]), " -> ".join([t.text_Y if i == 0 else t.text for i, t in enumerate(texts_2)]), texts_3[0].text]))
